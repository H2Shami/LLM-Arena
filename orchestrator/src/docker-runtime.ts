...
import { GeneratedCode, DockerContainer } from './types.js';
...
export class DockerRuntime {
  ...
  private containers: Map<string, { container: Docker.Container; ip: string; hostPort: number }>;
   ...
   */
  async createRun(runId: string, code: GeneratedCode): Promise<DockerContainer> {
   ...
   */
  private async prepareWorkspace(dir: string, code: GeneratedCode): Promise<void> {
   ...
   */
  private async buildPhase(runId: string, workspaceDir: string): Promise<void> {
   ...
   */
  private async runPhase(runId: string, workspaceDir: string): Promise<{ container: Docker.Container; ip: string; hostPort: number }> {
   ...
   */
  async isHealthy(ip: string): Promise<boolean> {
    try {
   ...
   */
  getInternalUrl(runId: string): string | null {
    const entry = this.containers.get(runId);
